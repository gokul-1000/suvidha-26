...
import { Plus, Trash2, CreditCard, TestTube, Droplet } from "lucide-react";

const WaterHub = () => {
  const navigate = useNavigate();
  const { t } = useLanguage();
  const { success, error } = useNotification();
  const { schemes, advisories, loading } = usePublicData("WATER");
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        >
          <h3 className="text-lg font-bold text-blue-900 mb-2">
            Water Department Advisories
          </h3>
          {loading ? (
            <p className="text-blue-800">Loading advisories...</p>
          ) : latestAdvisories.length ? (
            <ul className="space-y-2">
              {latestAdvisories.map((adv) => (
                <li key={adv.id} className="text-blue-800">
                  {adv.message}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-blue-800">No advisories right now.</p>
          )}
        </motion.div>
        ...
        >
          ...
          {loading ? (
            ...
            <p className="text-gray-700">
              No schemes available for Water currently.
            </p>
          )}
        </motion.div>
      </main>
